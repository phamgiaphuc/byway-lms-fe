...
import { updateCourseSchema, type Course, type UpdateCourseSchema } from "@/types/course";
import { useUpdateCourseById } from "@/hooks/tanstack-query/use-course";
...
const CourseSettingsPage = ({ course }: CourseSettingsPageProps) => {
  const { data: categories } = useGetCategories();
  const { mutate } = useUpdateCourseById();
  ...
  const form = useForm({
    ...
    values: {
      ...
      level: course.level as UpdateCourseSchema["level"],
    },
  });

  const isFree = form.watch("isFree");

  const onSubmit = (values: UpdateCourseSchema) => {
    mutate(values, {
      onSuccess: (resposne) => {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="relative space-y-6 py-4">
        <div className="flex items-center justify-between px-4">
          <div>
            <h1 className="text-2xl font-semibold">Update the course</h1>
            <span className="text-muted-foreground">Change the course information.</span>
          </div>
